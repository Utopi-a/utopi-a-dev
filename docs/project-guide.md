...
| `/work`, `/lab` | `(public)` | 不要 | プレースホルダー |
| `/blog` | `(public)` | 不要 | 公開記事一覧（プレースホルダー） |
| `/dashboard` | `(app)` | **必要**（`proxy.ts`） | セッション表示のみ |
| `/blog/manage` | `(app)` | **必要** | 記事の作成・編集（プレースホルダー） |
| `/settings` | `(app)` | **必要** | プレースホルダー |

`src/proxy.ts` が `/dashboard`, `/blog/manage`, `/settings` をマッチし、未ログインなら `/` へリダイレクトします。

**ブログの URL 設計**: 公開用 `/blog` と管理用 `/blog/manage` を分け、同じパスで route group が衝突しないようにしています。
...
| ブログ CRUD | `src/features/blog/<ユースケース>/` + `(app)/blog/manage/` |
