...
export function HomeStorageWarning({
  ...
  const message = isCurrentlyExceeded
    ? `帳簿上の現在残数が ${formatStockQuantity({ quantity: currentStock })}で、自宅保管の目安（${formatStockQuantity({ quantity: homeStorageRoundLimit })}）を超えています。`
    : `過去に帳簿残数が ${formatStockQuantity({ quantity: peakStock })}まで達しており、自宅保管の目安（${formatStockQuantity({ quantity: homeStorageRoundLimit })}）を超えた時期があります。`;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        ...
        個が目安です。射撃場での消費や譲渡・廃棄を記録すれば残数は減るため、帳簿と実態が一致していれば問題ありません。
      </p>
    </div>
  );
}
