...
export function comparePrefectures({ left, right }: { left: string; right: string }): number {
  const leftIndex = PREFECTURE_INDEX.get(normalizePrefecture({ prefecture: left })) ?? 999;
  const rightIndex = PREFECTURE_INDEX.get(normalizePrefecture({ prefecture: right })) ?? 999;
