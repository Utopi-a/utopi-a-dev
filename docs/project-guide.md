...
環境構築フェーズは完了しており、**アプリの本機能（日記 CRUD・課金など）はこれから** 実装する前提の骨格です。
...
| 手段 | 使う場面 |
| --- | --- |
| **Server Actions** | ログイン後画面からの日記作成・設定変更など、Next 内部の操作 |
...
│   ├── features/            # 機能ドメイン（auth, diary, billing 等）
...
Next.js の **App Router** がアプリの母艦です。Route Group で公開領域とログイン後領域を分けています。
...
| `/work`, `/lab`, `/blog` | `(public)` | 不要 | プレースホルダー |
| `/dashboard` | `(app)` | **必要**（`proxy.ts`） | セッション表示のみ |
| `/diary`, `/settings` | `(app)` | **必要** | プレースホルダー |

`src/proxy.ts` が `/dashboard`, `/diary`, `/settings` をマッチし、未ログインなら `/` へリダイレクトします。
...
├── layout/       # SiteHeader（公開）, AppShell（アプリ）
...
├── diary/
│   └── schema.ts           # Zod（日記入力）+ schema.test.ts
├── posts/
│   └── schema.ts           # 投稿用 Zod + テスト
├── billing/
│   └── index.ts            # プレースホルダー
├── profile/
│   └── index.ts
...
| `diary` | Zod schema のみ | `create-entry/` などユースケース単位のフォルダ |
| `posts` | Zod schema のみ | 同上 |
...
| `diary_entry` | `diary.ts` | 日記（未実装の CRUD 先） |
| `post` | `posts.ts` | 投稿（将来） |
...
1. GitHub OAuth は `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` が Doppler の `dev` にあるとき有効
2. セッションは PostgreSQL（Better Auth テーブル）に保存
3. ログイン UI は**まだ無い**（Dashboard はセッションがあれば email 表示）
...
| `NEXT_PUBLIC_APP_URL` | クライアント側のベース URL |
...
- `src/lib/cn.test.ts`
- `src/features/diary/schema.test.ts`
- `src/features/posts/schema.test.ts`
...
| 日記 CRUD | `src/features/diary/<ユースケース>/` + `(app)/diary/` |
